...
- **Sidebar views** — choose the Drawers/Categories tabs or a drawer-scoped sidebar; sort items by insertion, name, size, or position; filter unassigned items by whether they fit the selected drawer
...
- **Copy / paste** — duplicate selected items across drawers with Ctrl+C / Ctrl+V
...
- **Configurable sidebar** — toggle drawer/category item counts; choose between area (20U) or dimension (5×4) item size display; control category default expansion (none, categorized, all); choose the sidebar layout
- **Heatmaps** — optional grid coloring modes for item height or density instead of category color
...
Requires [Bun](https://bun.sh). The app is built with Vite, so Node.js 18+ with another package manager should also work if dependencies are installed.
...
| `gdp-project-{id}` | Full data for each project, including pending items |
